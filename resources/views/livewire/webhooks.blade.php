<div>
    <h3 class="font-bold text-lg text-indigo-700 leading-tight mb-6">{{ __('Webhooks') }}</h3>
    <div class="my-6">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $webhook->description }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ config('app.url').'/api/webhooks/'.$webhook->token }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <x-button type="danger" wire:click="delete('{{ $webhook->id }}')" text="{{ __('Delete') }}" />
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" colspan="4">{{ __('No webhooks found...') }}</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
